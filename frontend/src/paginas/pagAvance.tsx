...
const Avance: React.FC = () => {
    ...
    if (state && state.access_token !== undefined) {
      ...
      const fetchUsuario = async () => {
        try {
          ...
          if (!res.ok) throw new Error('Error al obtener usuario con datos locales');
  ...
  return (
    <Layout>
    <div>
      <h2>Avance Curricular - {carreraActual}</h2>
      <AvanceDisplay indice = {indice} access_token={access_token}/>
    </div>
    </Layout>
  );
}
